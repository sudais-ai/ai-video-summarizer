...
class VideoProcessor:
    ...
    def process_url(self, url):
        """Process video from URL (YouTube, Vimeo, etc.) and get real transcript"""
        try:
            ydl_opts = {
                'quiet': True,
                'no_warnings': True,
    ...
    def _extract_transcript(self, info):
        """Extract transcript from subtitles or automatic captions"""
        transcript_text = ""
    ...
    def _build_content_from_metadata(self, info):
        """Build content from video metadata when transcript unavailable"""
        parts = []
    ...
    def process_local_video(self, filepath):
        ...
        try:
            ...
            return {
                'transcript': f"Content from uploaded video file: {filename}. This video contains educational content.",
                'segments': [],
                'duration': 0,
                'title': filename,
                'language': 'en'
